...
func (daemon *Daemon) FileCacheDelete(jsonfilename string)(deleteresponses []*types.FileCacheDeleteResponseItem,err error){
	//hash := sha256.New()
	//hash.Write([]byte(Orig))
	//md := hash.Sum(nil)
	//mdStr := hex.EncodeToString(md)
	//pth:=filepath.Join(builder.Filecachejsonpath,jsonfilename)
	//_,err=os.Stat(pth)
	//if os.IsNotExist(err){
	//	logrus.Debug("FileCacheDelete file is not exist ",jsonfilename)
	...
	filemetadata,err=daemon.FromDisk(jsonfilename)
	if err!=nil{
		logrus.Debug("FileCacheDelete fromdisk error: ",err)
		if os.IsNotExist(err){
			logrus.Debug("FileCacheDelete file is not exist ",jsonfilename)
			deleteresponse:=&types.FileCacheDeleteResponseItem{
				Notexist:jsonfilename+"is not exist",
			}
			deleteresponses=append(deleteresponses,deleteresponse)
		}
		return
	...
	_,err=daemon.filecache.DelCopyInfo([]string{filemetadata.Orig})
	...
	//	}
	deleteresponse:=&types.FileCacheDeleteResponseItem{
		Orig:filemetadata.Orig+":JsonFileName "+jsonfilename,
	}
	deleteresponses=append(deleteresponses,deleteresponse)
	return
